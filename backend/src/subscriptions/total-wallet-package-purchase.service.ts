...
export class TotalWalletPackagePurchaseService {
  ...
  async purchase(
    ...
    const activation = await this.runSerializable(async (transaction) => {
      ...
      if (existing) {
        if (
          ...
          throw new ConflictException(
            'The package purchase request key was already used with different request data.',
          );
        }
      ...
      if (rangeConfigured) {
        ...
        if (amount.lt(minimum)) {
          throw new ConflictException(
            'Package purchase amount is below the configured minimum.',
          );
        }
        if (
          ...
          throw new ConflictException(
            'Package purchase amount exceeds the configured maximum.',
          );
        }
        ...
        throw new ConflictException(
          'Package purchase amount must match the configured package price.',
        );
      }
      ...
      const sourceKey = `PACKAGE_PURCHASE:${actor.id}:${dto.requestKey}`;
      ...
      await transaction.$executeRaw(Prisma.sql`
        INSERT INTO ledger_transactions (
          ...
          ${sourceKey},
          'TOTAL_WALLET_PACKAGE_PURCHASE',
          ${dto.requestKey},
          ${currency},
          ${actor.id},
          ${`Package ${packageItem.packageDisplayName} purchased from Total Wallet.`},
          ${JSON.stringify({
      ...
      if (!fundingTransaction) {
        throw new ServiceUnavailableException(
          'Package purchase funding transaction could not be established.',
        );
      }
      ...
      if (this.countNumber(entryCounts[0]?.total) > 0) {
        throw new ServiceUnavailableException(
          'Package purchase funding exists without its subscription record; reconciliation requires investigation.',
        );
      }
      ...
      await insertTotalWalletEvent(transaction, {
        ...
        insufficientMessage:
          'Total Wallet balance is insufficient for this package purchase.',
      });
      ...
      await this.insertEntry(
        ...
        'DEBIT',
        requestedAmount,
        `Package ${packageItem.packageDisplayName} funded from authoritative Total Wallet.`,
      );
      await this.insertEntry(
        transaction,
        fundingTransaction.id,
      ...
      await transaction.auditLog.create({
        data: {
          ...
          description:
            'USER purchased and activated a package directly from authoritative Total Wallet.',
          metadata: {
            source: 'TOTAL_WALLET_PACKAGE_PURCHASE',
            requestKey: dto.requestKey,
            userId: actor.id,
            packagePlanVersionId: packageItem.packagePlanVersionId,
            packagePlanItemId: packageItem.packagePlanItemId,
    ...
    return {
      ...
      message: downstream.downstreamPending
        ? 'Package purchased from Total Wallet. One or more downstream earning stages remain safely recoverable.'
        : 'Package purchased and activated from Total Wallet.',
    };
  }
  ...
  private async ensureAccount(
    ...
    if (
      ...
      throw new ServiceUnavailableException(
        'Package purchase ledger account semantics are inconsistent.',
      );
    }
  ...
  private async assertBalanced(
    ...
    if (entries.length !== 2 || debit.lte(0) || !debit.equals(credit)) {
      throw new ServiceUnavailableException(
        'Package purchase ledger transaction is not balanced.',
      );
    }
  }
