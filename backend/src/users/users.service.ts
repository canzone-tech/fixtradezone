...
export class UsersService {
  ...
  async create(
    ...
    try {
      return await this.prisma.$transaction(
        async (transaction) => {
          const defaultRole =
            await this.rbacBootstrapService.ensureDefaultUserRole(
              transaction,
            );
          ...
          await transaction.auditLog.create({
            data: {
              ...
              entityId: user.id,
              description:
                'Administrator created a platform user.',
              metadata: {
                source: 'ADMIN_API',
  ...
  async replaceRoles(
    ...
    if (
      previousRoleNames.includes(SUPER_ADMIN_ROLE_NAME)
    ) {
      throw new ForbiddenException(
    ...
    if (
      dto.roleNames.includes(SUPER_ADMIN_ROLE_NAME)
    ) {
      throw new ForbiddenException(
      ...
      async (transaction) => {
        ...
        await transaction.auditLog.create({
          data: {
            ...
            entityId: userId,
            description:
              'Administrator replaced user role assignments.',
            metadata: {
              source: 'ADMIN_API',
