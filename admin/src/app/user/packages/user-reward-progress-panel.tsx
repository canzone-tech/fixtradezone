...
async function fetchRewards(): Promise<MyRewardsResponse> {
  ...
  const body = (await response.json().catch(() => ({}))) as
    MyRewardsResponse | ErrorPayload;
  if (!response.ok) {
    throw new Error(
      message(body as ErrorPayload, "Unable to load package rewards."),
    );
  }
    ...
    <section className={styles.panel}>
      ...
      <div className={styles.header}>
        <div>
          ...
          <p>
            Only settled package rewards are shown as earnings. Cap and
            lifecycle progress comes from immutable subscription policy
            snapshots.
          </p>
        </div>
      ...
      <div className={styles.stateGrid}>
        {states.map((state) => {
          ...
          const percent =
            limit > 0 ? Math.min(100, (consumed / limit) * 100) : 0;
          return (
            <article className={styles.stateCard} key={state.subscriptionId}>
              ...
              <div className={styles.capNumbers}>
                ...
                <div>
                  ...
                  <strong>{amountLabel(state.capLimit, state.currency)}</strong>
                </div>
                <div>
                  <small>REMAINING</small>
                  <strong>
                    {amountLabel(state.capRemaining, state.currency)}
                  </strong>
                </div>
              </div>
              <div
                className={styles.progressTrack}
                aria-label={`Cap ${percent.toFixed(1)} percent consumed`}
              >
              ...
              <dl>
                <div>
                  ...
                  <dd>
                    {state.status === "ACTIVE"
                      ? formatPlatformDateTime(state.nextRewardAt)
                      : "—"}
                  </dd>
                </div>
                <div>
                  <dt>Natural package day</dt>
                  <dd>{state.nextRewardDayNumber}</dd>
                </div>
                <div>
                  <dt>Cycle</dt>
                  <dd>
                    {state.nextCycleNumber} / day {state.nextCycleDay}
                  </dd>
                </div>
                <div>
                  <dt>Cap basis</dt>
                  <dd>{state.capBasis.replaceAll("_", " ")}</dd>
                </div>
              </dl>
              {state.blockedReason ? (
                <p className={styles.blocked}>{state.blockedReason}</p>
              ) : null}
              {state.completionReason ? (
                <p className={styles.complete}>
                  {state.completionReason.replaceAll("_", " ")}
                </p>
              ) : null}
            </article>
          );
          ...
          <table>
            ...
            <tbody>
              {events.map((event) => (
                <tr key={event.id}>
                  <td>
                    {event.rewardLocalDate}
                    <small>Day {event.rewardDayNumber}</small>
                  </td>
                  <td>
                  ...
                  <td>
                    {event.selectedRate}%
                    <small>{event.rewardRateMode.replaceAll("_", " ")}</small>
                  </td>
                  <td>
                    {amountLabel(event.postedReward, event.currency)}
                    {event.clippedToCap ? (
                      <small>Clipped to exact cap headroom</small>
                    ) : null}
                  </td>
                  <td>
                  ...
                  <td>
                    {event.completionReason
                      ? event.completionReason.replaceAll("_", " ")
                      : "ACTIVE"}
                  </td>
                </tr>
              ))}
