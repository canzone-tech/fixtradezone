...
15. `DEPOSITS-FOUNDATION.md` — locked DEP-01 receiving-account, deposit, TXID and manual-review contract.
16. `PACKAGE-SUBSCRIPTION-ACTIVATION.md` — canonical SUB-02 activation/accounting/idempotency contract and 2026-08-27 local acceptance evidence.
17. `CHANGELOG.md` — session and milestone history.
...
For package subscription/activation behavior, `PACKAGE-SUBSCRIPTION-ACTIVATION.md` is the active milestone contract and supersedes older SUB-01-only assumptions such as globally fixed `SINGLE_ACTIVE` or `PAYMENT_APPROVED` execution.
